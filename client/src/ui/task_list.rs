...
use crate::{
    mid::{PropNameKey, State, Task, TaskKey, ViewKey},
    ui::{report_error, task_list::task_popup::CloseError},
};
...
impl TaskList {
    ...
    pub fn prune_list(&mut self, state: &State) {
        ...
        let current_task = self
            .get_task_idx()
            .and_then(|s| self.shown_tasks.get(s).cloned());
        self.shown_tasks
            .extract_if(|k| state.task_get(*k).is_err())
            .for_each(|key| {
                if Some(key) == current_task {
                    did_switch = true;
                };
                if !did_switch {
                    removed_count += 1;
        ...
        if len == 0 {
            ...
            let new_idx = self.get_task_idx().map(|idx| {
                idx.saturating_sub(removed_count)
    ...
    pub fn rebuild_list(&mut self, state: &State) {
        ...
        self.source_views
            .iter()
            .flat_map(|key| state.view_get(*key).ok())
            .map(|view| set.extend(view.tasks.iter().flatten()))
            .last();
    ...
    fn set_task_idx(&mut self, idx: Option<usize>) {
        self.table_state.select(idx.map(|i| i + 1)); // add 1 to ignore table top bar
    }
    fn get_task_idx(&mut self) -> Option<usize> {
        self.table_state.selected().map(|i| i - 1) // minus 1 so that calculations don't factor in table top bar
    }
    ...
    pub fn shift(&mut self, amt: isize, wrap: bool) {
        ...
        let bounded_index = if wrap {
            // if wrapping, add signed and modulo length
            (len.saturating_add_signed(new_index)) % len
        } else {
            // if not wrapping, clamp to bounds
        ...
        if let Some(task_popup) = &mut self.task_popup {
            ...
            return match task_popup.handle_term_event(state, event) {
                ...
                Err(err) => {
                    ...
                    if let Some(err) = err {
                        match err {
                            CloseError::NoTaskError(err) => {
                                log::error!("attempted to delete a task: {err:?}")
                            }
                            CloseError::ModifyTaskError(err) => {
                                log::error!("attempted to modify a task but got error: {err:?}")
                            }
    ...
    pub fn render(&mut self, state: &State, block: Block<'_>, area: Rect, buf: &mut Buffer) {
        ...
        self.prune_list(state);
        let valid_tasks = self
            .shown_tasks
            .iter()
            .flat_map(|key| state.task_get(*key).ok().map(|t| (key, t)));

        // get selected column property name
        let prop_col_key = self.prop_cols.get(self.col_idx);
        // string form of key
        let prop_col_name =
            prop_col_key.map(|&key| state.prop_get_name(key).expect("invalid prop name key"));
        ...
        let mut data_rows = valid_tasks
            .map(|(task_key, task)| {
                let mut text_style: Style = if task.completed {
                    COMPLETED_TEXT_COLOR.into()
                } else {
                    TEXT_COLOR.into()
                };
            ...
            .peekable();
        if data_rows.peek().is_some() {
            // if there are tasks to render
            ...
            let no_view_text = Text::from(vec![Line::from(vec![
                "No Tasks, Have you Selected a View?".into(),
            ])]);
